...
{	
    ...
    private function RemoveBuildingFromQueue($QueueID)
    {
        ...
        $BuildEndTime   = intval($CurrentQueue[$QueueID - 2][3]); // Force integer conversion
        ...
        {               
            if ($ID < $QueueID - 1) {
                ...
                $BuildEndTime       += intval(BuildFunctions::getBuildingTime($USER, $PLANET, $ListIDArray[0], NULL, $ListIDArray[4] == 'destroy', $ListIDArray[2]));
                $ListIDArray[3]     = $BuildEndTime;
                $NewQueueArray[]    = $ListIDArray;              
            }
        }
    ...
    private function getQueueData()
    {
        ...
        foreach($buildQueue as $BuildArray) {
            ...
            // Force integer conversion for timestamps
            $buildEndTime = intval($BuildArray[3]);
            
            $scriptData[] = array(
                'element'   => $BuildArray[0],
                'level'     => $BuildArray[1],
                'time'      => intval($BuildArray[2]),
                'resttime'  => ($buildEndTime - TIMESTAMP),
                'destroy'   => ($BuildArray[4] == 'destroy'),
                'endtime'   => _date('U', $buildEndTime, $USER['timezone']),
                'display'   => _date($LNG['php_tdformat'], $buildEndTime, $USER['timezone']),
            );
        }
